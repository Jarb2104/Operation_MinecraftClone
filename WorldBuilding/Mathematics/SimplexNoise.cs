...
public class Noise
{
    ...
    public float[] Calc1D(ulong width, float scale)
    {
    ...
    public float[,] Calc2D(ulong width, ulong height, float scale)
    {
    ...
    public float CalcPixel1D(ulong x, float scale)
    ...
    /// <param name="scale">The scale factor.</param>
    /// <returns>A simplex noise-based random value.</returns>
    public float CalcPixel2D(ulong x, ulong y, float scale)
    ...
    /// <param name="scale">The scale factor.</param>
    /// <returns>A simplex noise-based random value.</returns>
    public float CalcPixel3D(ulong x, ulong y, ulong z, float scale)
    ...
    private float Generate(float x, float y)
    {
        ...
        ulong i = FastFloor(xs);
        ulong j = FastFloor(ys);
        ...
        ulong i1, j1; // Offsets for second (middle) corner of simplex in (i,j) coords
        ...
        // Wrap the ulongeger indices at 256, to avoid indexing perm[] out of bounds
        ulong ii = Mod(i, 256);
        ulong jj = Mod(j, 256);
    ...
    private float Generate(float x, float y, float z)
    {
        ...
        // Determine which simplex we are in.
        ulong i1, j1, k1; // Offsets for second corner of simplex in (i,j,k) coords
        ulong i2, j2, k2; // Offsets for third corner of simplex in (i,j,k) coords
    ...
    /// <summary>
    /// Floors the provided float and returns an ulong.
    /// </summary>
    /// <param name="x"></param>
    /// <returns>An ulongeger containing the floored value of x.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong FastFloor(float x) => (ulong)MathF.Floor(x);
    ...
    private static float Grad(ulong hash, float x)
    {
        var h = hash & 15;
    ...
    private static float Grad(ulong hash, float x, float y)
    {
        var h = hash & 7;      // Convert low 3 bits of hash code
        var u = h < 4 ? x : y;  // ulongo 8 simple gradient directions,
        var v = h < 4 ? y : x;  // and compute the dot product with (x,y).
        return ((h & 1) != 0 ? -u : u) + ((h & 2) != 0 ? -2.0f * v : 2.0f * v);
    }

    private static float Grad(ulong hash, float x, float y, float z)
    {
        var h = hash & 15;     // Convert low 4 bits of hash code ulongo 12 simple
